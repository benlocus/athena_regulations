...
<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
			...
			<nav class="mt-8 flex items-center justify-center gap-2" aria-label="Search pagination">
				{#if data.page > 1}
					<a
						...
						class="rounded-md border border-border bg-surface px-4 py-2 text-sm font-medium text-text transition-colors hover:bg-background"
					>
						Previous
					</a>
				{/if}
	...
	{:else}
		<div class="py-16 text-center">
			<svg class="mx-auto h-16 w-16 text-text-muted/30" viewBox="0 0 24 24" fill="none">
				<circle cx="11" cy="11" r="7" stroke="currentColor" stroke-width="1.5" />
				<path d="M16 16l4.5 4.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
			</svg>
			<h1 class="mt-4 font-serif text-2xl font-bold text-primary">Search Regulations</h1>
			<p class="mt-2 text-text-muted">Enter a search term to find relevant regulation sections.</p>
		</div>
	{/if}
</div>
