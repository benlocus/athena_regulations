...
{#if bookmarks.length === 0}
	<div class="py-12 text-center">
		...
		<p class="text-muted-foreground">No bookmarks yet</p>
		...
		{#each bookmarks as bookmark (bookmark.id)}
			<li>
				<a
					...
					class="block px-4 py-3 transition-colors hover:bg-muted"
				>
					<div class="flex items-start justify-between gap-3">
						<div class="min-w-0 flex-1">
							...
							{#if bookmark.label}
								<p class="mt-1 text-sm text-muted-foreground">{bookmark.label}</p>
							{/if}
						</div>
